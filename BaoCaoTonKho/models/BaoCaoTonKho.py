...
class BaoCaoTonKho(models.Model):
    ...
    phieunhapkho_id = fields.Many2one("phieunhapkho", string="Phiếu nập kho")
    ...
    #     for item in self:
    #         ten_kho = item.sanpham_item_id.kho_id
    #         print('kho', ten_kho)
    #         item.tenkho= ten_kho
    ...
    def _compute_real_quantity(self):
        for item in self:
            related_phieunhapkho = self.env['phieunhapkho'].search([
                ('sanpham_id', '=', self.sanpham_item_id.id),
            ])
    ...
    def _compute_total_price(self):
        for i in self:
            tong = i.soluongthucte * i.sanpham_item_id.price
            i.totalprice = tong